...
<template>
  <div class="container mx-auto text-gray-600">
    <header v-if="story" class="relative">
      <img :src="story.content.media.filename" :alt="story.content.media.alt" class="rounded-lg aspect-square" />
      ...
      <div class="bg-white rounded-lg overflow-hidden shadow-md p-8 absolute w-1/2 top-4 right-0">
        ...
        <ul class="flex mb-8 border-b border-gray-400">
          <li class="flex items-center mr-4">
            <i class="i-carbon-time" />
            <span class="ml-2">{{ story.content.prep_time }}</span>
          </li>
          <li class="flex items-center">
            <i class="i-carbon-user" />
            <span class="ml-2">Serves {{ story.content.dinners }}</span>
          </li>
        </ul>
        <div class="prose">
